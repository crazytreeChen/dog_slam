...
def main():
    ...
    web_dir = os.getcwd()
    if not os.path.isfile(os.path.join(web_dir, 'map_viewer.html')) and \
       not os.path.isfile(os.path.join(web_dir, 'map_tracking_view.html')):
        print(f'[web] 警告: {web_dir} 中未找到 map_viewer.html 或 map_tracking_view.html，继续启动...',
              file=sys.stderr)
